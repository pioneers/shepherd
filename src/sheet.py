...
class Sheet:
    ...
    @staticmethod
    def write_security_breach(match_number, alliance, done):
        def bg_thread_work():
            try:
                Sheet.__write_security_breach(
                    match_number, alliance, done)
    ...
    @staticmethod
    def write_cheat_code(match_number, alliance, score):
        def bg_thread_work():
            try:
                Sheet.__write_cheat_code(
...
def main():
    ...
    try:
        ...
        result = (
            sheet.values()
            .get(spreadsheetId=CONSTANTS.SPREADSHEET_ID, range="Match Database!A1:B")
            .execute()
        )
